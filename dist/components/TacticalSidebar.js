...
export function TacticalSidebar({ user, links, logoUrl, onLogout, brandName, LinkComponent, translations, activeHref, homeHref = "/dashboard", menuAriaLabel = "Toggle Tactical Menu", }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("button", { "aria-label": menuAriaLabel, "aria-expanded": isOpen, "aria-controls": "tactical-sidebar-panel", onClick: () => setIsOpen(!isOpen), className: cn("fixed top-6 left-6 p-3 rounded-none bg-background/80 backdrop-blur-md shadow-lg border border-border hover:border-primary/40 hover:bg-muted transition-all duration-200 cursor-pointer active:scale-95 focus:outline-none focus:ring-1 focus:ring-primary/40", isOpen ? "z-[55]" : "z-40"), children: isOpen ? (_jsx(X, { className: "w-5 h-5 text-primary" })) : (_jsx(Menu, { className: "w-5 h-5 text-foreground" })) }), isOpen && (_jsx("div", { "aria-hidden": "true", onClick: () => setIsOpen(false), className: "fixed inset-0 z-[45] bg-black/70 backdrop-blur-sm transition-opacity duration-300 animate-in fade-in" })), _jsxs("aside", { id: "tactical-sidebar-panel", role: "navigation", "aria-label": "Tactical Navigation", className: cn("fixed inset-y-0 left-0 z-50 w-80 bg-background border-r border-border shadow-2xl flex flex-col p-6 transition-transform duration-300 ease-in-out transform rounded-none", isOpen ? "translate-x-0" : "-translate-x-full"), children: [_jsxs("div", { className: "flex justify-between items-center mb-8 pt-12 border-b border-border pb-4", children: [_jsxs(Link, { href: homeHref, onClick: () => setIsOpen(false), className: "flex items-center gap-3", children: [logoUrl ? (_jsx("img", { src: logoUrl, alt: "Logo", className: "w-6 h-6 object-contain filter drop-shadow-[0_1px_3px_rgba(0,0,0,0.3)]" })) : (_jsx("div", { className: "w-6 h-6 bg-primary/10 border border-primary/30 flex items-center justify-center", children: _jsx(Shield, { size: 12, className: "text-primary" }) })), _jsx("span", { className: "font-mono text-xs font-black uppercase tracking-[0.2em] text-foreground", children: user.tenantId || brandName || t.brandFallback })] }), _jsx("button", { "aria-label": "Close navigation", onClick: () => setIsOpen(false), className: "p-1.5 hover:bg-muted border border-border rounded-none text-muted-foreground hover:text-foreground transition-colors cursor-pointer", children: _jsx(X, { className: "w-4 h-4" }) })] }), _jsx("nav", { className: "flex-1 flex flex-col gap-2 overflow-y-auto", children: links.map((link) => {
